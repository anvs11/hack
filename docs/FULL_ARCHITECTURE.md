...
| `/sources` | `SourcesPage` | Read-only состояние источников |
...
| Источники | `GET /api/sources` | Read-only список |
...
| `Источники` | Открывает read-only список | listSources |
...
| `Подтверждение в источнике` | Открывает официальный URL lifecycle event | Нет |
...
**Факт:** текущий frontend только читает данные. В нём отсутствуют:

- кнопка запуска replay/live анализа;
- форма подтверждения/исправления решения;
- просмотр всей истории анализов и решений;
- создание/редактирование/пауза источника;
- ручное добавление/редактирование/скрытие публикации;
- создание кейса НПА;
- привязка публикации к кейсу;
- добавление lifecycle event;
- дата-фильтры `published_from`/`published_to`;
- серверная генерация дайджеста.

Эти действия нельзя получить одной реализацией backend: соответствующие элементы и
...
- Страница `Источники` прямо помечена как demo без редактирования.
...
- [x] Все четыре текущих API-вызова frontend описаны.
