...
> Актуализация 2026-09-06: обычный runtime больше не импортирует синтетический seed,
> публичный `/api/demo/seed` удалён, рабочая БД содержит только реально собранные или
> вручную добавленные материалы. `data/seed` и frontend mocks сохранены исключительно
> как fixtures изолированных автотестов. Досье нормативного документа создаётся
> человеком на основании официального источника и связывает несколько публикаций об
> одном документе; collector сам юридическую стадию не подтверждает.
...
общие и различающиеся факты. Такой ответ остаётся подсказкой человеку, а не
автоматическим удалением. Прогонять генеративную модель на всех парах polling
...
Техническая очередь читается через `GET /api/duplicate-candidates`; параметр
`publication_id` выбирает пары открытой карточки. Каждый человеческий вердикт
сохраняется новой `DuplicateReview`. Отдельного пользовательского экрана очереди
нет: проверка показывается прямо в карточке, а подтверждённый duplicate больше не
занимает карточку мониторинга и становится дополнительным первоисточником события.
...
Дайджест — производное представление уже проверенных важных публикаций и досье для
...
Отдельного endpoint серверного снимка нет: web-preview по-прежнему строится из read
API. Telegram-доставка реализована отдельно: пользователь включает её через
`/api/me/telegram-digest-settings`, а worker после анализа отправляет только новые
релевантные версии и пишет `TelegramDigestDelivery`. Email-рассылка отложена. Достоверный аудит создания/редактирования источников,
...
| `TelegramDigestPreference` | Включение доставки и минимальный приоритет пользователя | Сам пользователь через Mini App |
| `TelegramDigestDelivery` | Уже доставленную пользователю версию AI-анализа | 15-минутный worker после успешной отправки |
...
| `prioritization` | Детерминированный score при полных критериях, fallback-приоритет LLM и обязательный review при `null` |
...
| Нормативный документ | `GET /api/regulatory-cases/{id}` | Досье и timeline |
| Нормативный документ | `GET /api/publications/{id}`, `GET /api/sources` | Метаданные связанных дополнительных материалов |
| Нормативный документ | `POST /api/regulatory-cases/{id}/lifecycle-events` | Новое официально подтверждённое событие |
...
| Профиль | `GET /api/me`, `PATCH /api/me` | Роль, права, личный режим и стартовые фильтры |
| Telegram отчёт | `POST /api/telegram/report-deliveries` | Доставка отчёта текущему подписанному пользователю |
| Telegram автоотчёт | `GET/PATCH /api/me/telegram-digest-settings` | Включение и минимальный приоритет |
...
Лента и backend сортируют материалы по дате публикации от новых к старым, затем по
приоритету и `id`. Поэтому новое событие всегда появляется сверху до пагинации.
...
```text
Мониторинг         → текущие события, по одной карточке на событие
Публикация         → исходник, несколько подтверждающих ссылок и объяснение AI
Нормативный документ → долгоживущая история и стадии документа
...
| `Кратко / Подробно` | Меняет личную плотность интерфейса | updateMyPreferences |
| `Нормативные документы` | Открывает реестр долгоживущих досье | listRegulatoryCases |
...
| `Добавить в досье` | Выбирает нормативный документ и создаёт идемпотентную связь | listRegulatoryCases + linkPublicationToCase |
...
| `Объединить источники` / `Оставить связанными` / `Это разные события` | Сохраняет вердикт прямо в карточке | listDuplicateCandidates с publication_id + createDuplicateReview |
...
- Раздел `Нормативные документы` показывает реальный список досье, а из публикации
  можно привязать материал к выбранному документу.
- Demo-метка на карточке `Источники` означает происхождение данных, а не запрет редактирования.
- Страница `Дайджест` строит web-preview из read API; ручная и автоматическая
  Telegram-доставка работают независимо от хранения версии снимка.
...
Контрактные операции создания и изменения источника реализованы. В API v0.5.0
