  ...
  return (
    <section>
      <header className="page-heading">
        <div><p className="eyebrow">Демо-режим</p><h1>Дайджест для руководителя</h1><p className="page-description">{localDigest.label} · {localDigest.preparedFor}</p></div>
      </header>
      ...
      {localDigest.items.length === 0 ? (
        ...
        <ol className="digest-list">
          {localDigest.items.map((item, index) => (
            <li key={item.id}><span className="digest-index">{String(index + 1).padStart(2, '0')}</span><div><h2><Link to={item.href}>{item.title}</Link></h2><p>{item.reason}</p></div></li>
          ))}
        </ol>
      )}
    </section>
